...
from models import solid_task_master, solids_task_mapping
...
async def update_solids_task_mappings(year: int, db: Session = Depends(get_db)):
    ...
    for record in all_records:
        ...
        for period in range(1,14):
            new_record = solids_task_mapping(solids_task_id = record.solids_task_id, period=period, year=year, value=0.0)
            db.add(new_record)
            db.commit()
...
def update_solid_rate_mapping_records(payload: SolidsTaskMappingSchemaPayload, db: Session = Depends(get_db)):
    ...
    try:
        for item in data:
            ...
            db.query(solids_task_mapping).filter(solids_task_mapping.solids_task_id == item.solids_task_id, solids_task_mapping.year==item.year, solids_task_mapping.period==item.period).update(
